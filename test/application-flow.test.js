...
import learningVideoRequestCommand from '../src/commands/dorm/learningVideoRequest.js';
import { setUserInfo } from '../src/utils/userRegistry.js';
...
test('학습 영상 신청은 층, 설명, 선택 링크, 시간을 입력받는다', async () => {
  await updateSettings('guild-a', (settings) => {
    settings.channels = { 학습영상신청: { channelId: 'video-channel' } };
  });
  await setUserInfo('guild-a', 'user-a', { studentId: '70707', name: '홍길동', room: '707' });
  let modal;

  await learningVideoRequestCommand.execute({
    guildId: 'guild-a',
    channelId: 'video-channel',
    user: { id: 'user-a' },
    member: { displayName: '홍길동' },
    showModal: async (value) => { modal = value.toJSON(); },
  });

  const inputs = modal.components.map((row) => row.components[0]);
  assert.deepEqual(inputs.map(({ custom_id }) => custom_id), ['floor', 'description', 'reference', 'duration']);
  assert.deepEqual(inputs.map(({ required }) => required), [true, true, false, true]);
  assert.match(inputs[2].placeholder, /줄바꿈/);
});

test('학습 영상 신청은 링크가 없어도 표시하고 여러 링크를 줄바꿈으로 구분한다', () => {
  const withoutLinks = videoRequestEmbed({
    id: 'video-a', requesterDisplayName: '홍길동', floor: '3층', duration: '20분', description: '졸작 테스트', reference: '',
  }).toJSON();
  const withLinks = videoRequestEmbed({
    id: 'video-b', requesterDisplayName: '홍길동', floor: '3층', duration: '20분', description: '영상 시청',
    reference: 'https://youtu.be/first\nhttps://www.youtube.com/watch?v=second',
  }).toJSON();

  assert.equal(withoutLinks.fields.some(({ name }) => name === '영상 링크'), false);
  assert.equal(withoutLinks.fields.find(({ name }) => name === '학습할 층').value, '3층');
  assert.equal(withoutLinks.fields.find(({ name }) => name === '설명').value, '졸작 테스트');
  assert.match(withLinks.fields.find(({ name }) => name === '영상 링크').value, /링크 1 열기/);
  assert.match(withLinks.fields.find(({ name }) => name === '영상 링크').value, /링크 2 열기/);
});
